...
| Scripting language for protocols (Lua, Python) | Rejected as the artifact 2026-08-26 | The stored, checked protocol stays a total and bounded data AST (FR-2.6). A deterministic, terminating compile-time frontend (Starlark-style) may be added for authoring when a real protocol needs macros or computed values (ARCHITECTURE §4.2). |
