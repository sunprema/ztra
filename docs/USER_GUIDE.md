...
| `ztra run <branch> --yes --driver otsim` | The same run, but each segment also executes inside **Opentrons' own simulator** (point `ZTRA_OT_SIM_OT2`/`FLEX` at a vendor venv; needs apiLevel ≥ 2.22). The vendor engine's per-well liquid tracking is checked against ztra's at every pause — any disagreement aborts the run, because it means ztra's physics is wrong. Ideal pipettes, no fault injection: the fake driver rehearses failure, this one cross-checks correctness. |
